...
def normHema(im):
    im = (im + 1.6002805331796792) / (-0.5680482059939732 + 1.6002805331796792)
    return im


def normDab(im):
    im = (im + 1.2493565290878947) / (-0.33088655029237873 + 1.2493565290878947)
    return im
...
def gethalo(im, im_labeled, membrane, max_dab):
    d = 10
    ...
    sub_membrane_complete = sub_membrane_complete/max_dab
    completeness = np.mean(sub_mem[sub_membrane_complete > 0].flatten())
    staining  = np.mean(sub_im[sub_membrane_complete > 0].flatten())
    return completeness, staining

def membraneECADHotSpot(filename, imgCoords):
    slide = openslide.OpenSlide(filename)
    level_lowRes = slide.level_count - 3
    level_highRes = level_lowRes - 8
    ...
    # trash_, trash_, mean_dab, dab_std = getHemaDabNormParamROI(im, mean_hema, std_hema, mean_dab, std_dab)

    ihc_hed = rgb2hed(im)
    hema = opening(normHema(ihc_hed[:, :, 0]), disk(7))
    dab = normDab(ihc_hed[:, :, 2])

    hema_segmentation = membraneHemaBioMarkerROI(hema, mean_hema - std_hema * 2).astype(np.uint8)

    membrane = dab > mean_dab - std_dab

    nucleus_membrane = membraneDabBioMarkerROI(im, dab, mean_dab - std_dab * 2.5)
    ...
    json_mid = {"applyMatrix": True,
                "segments": [],
                "closed": True,
                "strokeColor": [0, 0, 0],
                "fillColor": [245 / 255., 206 / 255., 66 / 255., 0.5],
                "strokeScaling": False}
    data = []
    c_pos = 0
    c_neg = 0

    im_pos = np.zeros(hema_segmentation.shape)
    im_neg = np.zeros(hema_segmentation.shape)
    for l in labels_hema_segmentation:
        aux = np.zeros(hema_segmentation.shape, dtype=np.bool)
        aux[hema_segmentation == l] = True
        contours, hierarchy = cv2.findContours(aux.astype(np.uint8) * 255, cv2.RETR_TREE,
                                               cv2.CHAIN_APPROX_SIMPLE)[-2:]
        json_neg["fillColor"][3] = 0.5

        im_neg[hema_segmentation == l] = 255
        for i, contour in enumerate(contours):
            c_neg = c_neg + 1
            xv = contour[:, 0, 0]
            yv = contour[:, 0, 1]
    ...
    for kk, l in enumerate(labels_nucleus_membrane_segmentation):
        ...
        # sub_membrane_complete = np.bitwise_xor(binary_dilation(aux.copy(), disk(15)), aux)
        # v = np.mean(membrane[sub_membrane_complete>0].flatten())
        v, trash_ = gethalo(dab, aux, membrane, max_dab)
...
def membraneHERHotSpot(filename, imgCoords):
    ...
    n_strong_dab = np.sum(dab.flatten()>0.95)
    print(n_strong_dab, n_strong_dab/im.size)
    im_pos = np.zeros(dab.shape)
    max_dab = np.max(dab.flatten())

    #Decidir que algoritmo usar (umbrales con gaussian mixed model)
    gmm = GaussianMixture(n_components=2, covariance_type='full')
    f = dab.flatten().reshape(-1, 1)
    gmm.fit(f)


    th_bg = np.min(gmm.means_)
    drift = (th_bg-0.90)
    if drift < 0:
      drift = 0
    th_type = np.max(gmm.means_) - drift
    ...
    if 0:
        gmm_x = np.linspace(0, 1.3, 256)
        gmm_y = np.exp(gmm.score_samples(gmm_x.reshape(-1, 1)))
        # Plot histograms and gaussian curves
        plt.hist(f,255,[0,1.3], normed=True)
        plt.plot(gmm_x, gmm_y, color="crimson", lw=4, label="GMM")
        plt.show()

    if th_type < 0.92:    #0 y 1+    #Pre-segmentacion de nucleos Hema
        hema_preseg = hema.copy()
        #Modificar nivel para menor o mayor sensibilidad de nucleos (menor nivel, mayor sensibilidad)
        nivel = 2
        for i in np.arange(0, nivel):
            hema_preseg[hema_preseg <= mean_hema] = 0
        ...
        #Decidir si los núcleos son o no tumorales en función de la variabilidad tumoral.

        nucleus_membrane = label(hema_preseg)
        nucleus_membrane_classified = nucleus_membrane.copy()
        labels_nucleus_membrane_segmentation = np.unique(nucleus_membrane.flatten())
        labels_nucleus_membrane_segmentation = labels_nucleus_membrane_segmentation[labels_nucleus_membrane_segmentation > 0]


        for kk, l in enumerate(labels_nucleus_membrane_segmentation):
            aux = np.zeros(hema_preseg.shape, dtype=np.bool)
            aux[nucleus_membrane == l] = True
            c, s = gethalo(dab, aux, membrane_complete, np.max(dab_membrane[dab_membrane > 0].flatten()))
            s  = s - drift
            if s > th_1:
                nucleus_membrane_classified[nucleus_membrane == l] = 2
                index = 1
            else:
                nucleus_membrane_classified[nucleus_membrane == l] = 1
                index = 0
        ...
        for kk, l in enumerate(labels_nucleus_membrane_segmentation):
            aux = np.zeros(nucleus_membrane.shape, dtype=np.bool)
            aux[nucleus_membrane == l] = True
            c, s = gethalo(dab, aux, membrane, max_dab)
            s  = s - drift
    ...
    print(scores)
    if scores[3]/np.sum(np.array(scores[0:3])) > 0.1:
        scores[4] = '3+'
    elif scores[2]/np.sum(np.array(scores[0:3])) > 0.1:
        scores[4] = '2+'
    elif scores[1]/np.sum(np.array(scores[0:3])) > 0.1:
        scores[4] = '1+'
    else:
        scores[4] = '0'
...
if __name__ == '__main__':
    ...
    imgCoords = [{'x': 19873.777468542103, 'y': 80851.76595907542}, {'x': 20873.777468542103, 'y': 80851.76595907542}, {'x': 20873.777468542103, 'y': 81851.76595907542}, {'x': 19873.777468542103, 'y': 81851.76595907542}]
    ...
    '''
    imgCoords = [{'x': 27021.737354624354, 'y': 49820.627622646505}, {'x': 28021.737354624354, 'y': 49820.627622646505}, {'x': 28021.737354624354, 'y': 50820.627622646505}, {'x': 27021.737354624354, 'y': 50820.627622646505}]
    ...
    '''
    imgCoords = [{'x': 42771.90621869021, 'y': 41312.530107091734}, {'x': 43771.90621869021, 'y': 41312.530107091734}, {'x': 43771.90621869021, 'y': 42312.530107091734}, {'x': 42771.90621869021, 'y': 42312.530107091734}]
