...
def nuclearBioMarkerHotSpot(filename, imgCoords):
    ...
    negative = {"applyMatrix": True,
           "segments": [],
           "closed": True,
           "strokeColor": [0, 0, 0],
           "fillColor": [0.2, 0.51, 168 / 155, 0.5],
           "strokeScaling": False}
    positive = {"applyMatrix": True,
           "segments": [],
           "closed": True,
           "strokeColor": [0, 0, 0],
    ...
    json_dict = [negative, positive]
    text_dict = ["Negative Nuclei", "Positive Nuclei"]
    ...
    nucleus = label(hema_preseg)
    labels_nucleus_segmentation = np.unique(nucleus.flatten())
    labels_nucleus_segmentation = labels_nucleus_segmentation[
        labels_nucleus_segmentation > 0]

    for kk, l in enumerate(labels_nucleus_segmentation):
    ...
    nivel = 2
    for i in np.arange(0, nivel):
        dab_preseg[dab_preseg <= mean_dab - std_dab] = 0
        mean_dab = np.mean(dab_preseg[dab_preseg > 0].flatten())
        std_dab = np.std(dab_preseg[dab_preseg > 0].flatten())
        if debug:
            plt.subplot(1, 2, 1)
            plt.imshow(im)
            plt.subplot(1, 2, 2)
    ...
    dab_preseg[dab_preseg > 0] = 1
    dab_preseg = binary_fill_holes(dab_preseg)
    dab_preseg = binary_opening(dab_preseg, disk(7))

    nucleus = label(dab_preseg)
    labels_nucleus_segmentation = np.unique(nucleus.flatten())
    labels_nucleus_segmentation = labels_nucleus_segmentation[
        labels_nucleus_segmentation > 0]

    for kk, l in enumerate(labels_nucleus_segmentation):
        aux = np.zeros(dab_preseg.shape, dtype=np.bool)
        aux[nucleus == l] = True
        h = np.mean(dab_original[aux].flatten())
        index = 1
    ...
    if scores[0] + scores[1] == 0:
        ...
        score = scores[1] / (scores[1] + scores[0])
    return data, score
...
if __name__ == '__main__':
    filename = '/media/telemed/BDPATH_WEBIM1/PI0032WEB/P005-KI67-047-6.mrxs'
    imgCoords = [{'x': 40162.02072563602, 'y': 47611.98091953313}, {'x': 41562.02072563602, 'y': 47611.98091953313}, {'x': 41562.02072563602, 'y': 49011.98091953313}, {'x': 40162.02072563602, 'y': 49011.98091953313}]
    nuclearBioMarkerHotSpot(filename, imgCoords)
    '''
    ...
    print("0")
    imgCoords = [{'x': 57752.62666027365, 'y': 54068.82266015419}, {'x': 58752.62666027365, 'y': 54068.82266015419},
                 {'x': 58752.62666027365, 'y': 55068.82266015419}, {'x': 57752.62666027365, 'y': 55068.82266015419}]
    filename = '/media/telemed/BDPATH_WEBIM1/PI0032WEB/P029-HER-097-3.mrxs'
    membraneHERHotSpot(filename, imgCoords)

    print("0")
    imgCoords = [{'x': 42397.41969083141, 'y': 40339.46495259131}, {'x': 43397.41969083141, 'y': 40339.46495259131},
                 {'x': 43397.41969083141, 'y': 41339.46495259131}, {'x': 42397.41969083141, 'y': 41339.46495259131}]
    ...
    print("2+")
    imgCoords = [{'x': 44193.665219742616, 'y': 49884.791757154875}, {'x': 45193.665219742616, 'y': 49884.791757154875},
                 {'x': 45193.665219742616, 'y': 50884.791757154875}, {'x': 44193.665219742616, 'y': 50884.791757154875}]
    filename = '/media/telemed/BDPATH_WEBIM1/PI0032WEB/P202-HER-351-3.mrxs'
    membraneHERHotSpot(filename, imgCoords)

    print("3+")
    imgCoords = [{'x': 35928.98208672112, 'y': 83539.35500985188}, {'x': 36928.98208672112, 'y': 83539.35500985188},
                 {'x': 36928.98208672112, 'y': 84539.35500985188}, {'x': 35928.98208672112, 'y': 84539.35500985188}]
